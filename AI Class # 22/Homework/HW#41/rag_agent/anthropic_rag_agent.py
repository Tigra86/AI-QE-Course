...
if not VOYAGE_API_KEY:
    raise EnvironmentError("Missing VOYAGE_API_KEY")
...
FLAT_INDEX_PATH = STORE_DIR / "anthropic_faiss_flat.index"
HNSW_INDEX_PATH = STORE_DIR / "anthropic_faiss_hnsw.index"
VECTORS_PATH = STORE_DIR / "anthropic_vectors.npy"
META_PATH = STORE_DIR / "anthropic_metadata.json"
...
def embed_texts(texts: List[str], model: str = EMBEDDING_MODEL) -> np.ndarray:
    ...
    for i in range(0, len(texts), EMBED_BATCH_SIZE):
        ...
        result = voyage_client.embed(batch, model=model, input_type="document")
...
def search_index_hnsw(
    ...
    if not FAISS_AVAILABLE:
        raise RuntimeError("FAISS is not installed.")
    if index is None:
        raise RuntimeError("FAISS index is missing. Run: python anthropic_rag_agent.py build")
